...
public class ThirdPersonCamera : MonoBehaviour
{
	...
	float desiredCameraDistance = 25f;
	Quaternion desiredRotation;
	Quaternion currentRotation;
	float slerpRotationSpeed = .2f;
	Vector2 cameraEulerAngle = new Vector2(85f, 0f);
	...
	public void ManualTurn(float turnAxis)
	{
		//cameraEulerAngle += (Vector2)newRotation;
		//ConstrainAngles();
		//desiredRotation = Quaternion.Euler(cameraEulerAngle);
		//cameraEulerAngle.y += rotateSpeed * Time.unscaledDeltaTime * turnInput;
		cameraEulerAngle.y += rotateSpeed * Time.unscaledDeltaTime * turnAxis;
		ConstrainAngles();
